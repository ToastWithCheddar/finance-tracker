"""
Account Reconciliation Service (Merged)
Comprehensive reconciliation with smart discrepancy detection and resolution
Combines functionality from both basic and enhanced reconciliation services
"""
    ...
    async def schedule_daily_reconciliation(self, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            for account in accounts_to_reconcile:
                ...
                metadata['next_reconciliation'] = (datetime.now(timezone.utc) + timedelta(hours=1)).isoformat()
                account.account_metadata = metadata
            ...
            logger.info(f"Scheduled daily reconciliation for {len(accounts_to_reconcile)} accounts")
            return results
